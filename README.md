...
│   └── DATA_REQUIREMENTS.md    # data spec for the wheat and rice models
...
guesswork, and should not be offered to farmers until the model is retrained. `docs/DATA_REQUIREMENTS.md` specifies
the dataset for building dedicated wheat and rice models. Healthy leaves were not tested at all, so the false-alarm rate on a healthy
